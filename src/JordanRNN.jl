...
# Define the structure of an Jordan RNN.
type JordanNetwork
	...
	function JordanNetwork( inputSize::Int, hiddenSize::Int, outputSize::Int)
		wHI = rand( hiddenSize, inputSize) - .5
		wHC = rand( hiddenSize, outputSize) - .5
...
function JordanTrain!( network::JordanNetwork, inputs::TimeSeriesSamples, targets::TimeSeriesSamples)
	# Check dimensions.
	sizeContext = size( network.weightsHC, 2)
	...
	while epoch < network.maxEpochs && totalError > network.errorThreshold
		...
		for p in 1:length(inputs.samples)
			...
			for t in 1:length(sample)
				...
				targetT = target[:,t]
				aO, aH = JordanEvaluateHelper( network, inputActivation, contextLayer)
...
# Helper function to JordanEvaluate, which is used internally to feed forward.
function JordanEvaluateHelper( network::JordanNetwork, input::Vector{Float64}, contextLayer::Vector{Float64})
...
# Evaluate the Jordan RNN with the given input vector.
function JordanEvaluate( network::JordanNetwork, input::TimeSeriesSample)
	...
	contextLayer = zeros( Float64, size( network.weightsHC, 2))

	# Evaluate the Jordan RNN.
	target = Array( Float64, size( network.weightsOH, 1), size( input.sample, 2))
	for i in 1:size( input.sample, 2)
		target[:,i], aH = JordanEvaluateHelper( network, input.sample[:,i], contextLayer)

		# Update context layer based on output activation.
		contextLayer = network.mu * contextLayer + target[:,i]
	end
